...
export class TraitsService {
  ...
  getTrait(id: string): Observable<Trait> {
    return this.http.get<Trait>(`${this.dev_url}/trait/${id}`)
  }

  createTrait(item: Trait): Observable<Trait> {
    return this.http.post<Trait>(`${this.dev_url}/store/trait`, item)
  }

  updateTrait(item: Trait): Observable<Trait> {
    return this.http.put<Trait>(`${this.dev_url}/trait/modify/${item._id}`, item)
  }
